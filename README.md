...
- SVG の描画は **GPU**（vello/wgpu）。数万要素の精密な SVG も数十msで描画
  （GPU が使えない環境や特殊な SVG では自動的に CPU 描画へフォールバック）
...
### 初回起動時の「Windows によって PC が保護されました」について

本アプリはコード署名証明書を持たない個人開発のフリーソフトのため、ダウンロード後の
**初回起動時に1回だけ** Microsoft Defender SmartScreen の警告が表示されます。

1. 青い画面に「実行しない」しか見えない場合は **「詳細情報」** をクリック
2. 表示された **「実行」** ボタンをクリック

これで起動します。**この警告はユーザーごとに初回の1回だけ**で、2回目以降は表示されません。
また、アプリ内の自動更新で新バージョンに更新した場合も警告は再表示されません。

心配な場合は、実行前に `SHA256SUMS.txt` とファイルのハッシュが一致するか確認できます:

```powershell
Get-FileHash MSBT-yuina.exe   # PowerShell
```

> 補足: この警告は zip / exe / インストーラーといった配布形式に関係なく、署名のない
> 実行ファイルすべてに表示される Windows の仕様です（有料のコード署名証明書または
> Microsoft Store 経由の配布でのみ解消できます）。MSIX 形式は署名がないと Windows が
> インストール自体を拒否するため採用していません。
...
- GUI: egui / eframe
- 画像処理: image-rs
- SVGサポート: usvg でパースし、GPU は vello/wgpu、CPU フォールバックは resvg / tiny-skia で
  可視領域を表示解像度でラスタライズ（拡大しても鮮明・視野外はカリング）
